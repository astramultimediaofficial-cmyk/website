...
const AstraContext = createContext();

export const AstraProvider = ({ children }) => {
...
export const useAstraContext = () => {
  const context = useContext(AstraContext);
  if (!context) {
    throw new Error("useAstraContext must be used within an AstraProvider");
  }
  return context;
};
